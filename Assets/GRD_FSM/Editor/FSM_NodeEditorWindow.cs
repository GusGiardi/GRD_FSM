...
public class FSM_NodeEditorWindow : EditorWindow
{
    ...
    [SerializeField] private FSM_Manager _manager;
    ...
    public static void OpenWindow(FSM_Manager manager)
    {
        FSM_NodeEditorWindow window = GetWindow<FSM_NodeEditorWindow>();
        window.titleContent = new GUIContent("FSM_Editor(" + manager.gameObject.name + ")");

        window._instanceID = manager.GetInstanceID();
        window._manager = manager;
        window._serializedObject = new SerializedObject(manager);

        window._defaultSelectionPropertiesSubWindowArea.position = new Vector2(window.position.width - window._defaultSelectionPropertiesSubWindowArea.width, 0);
    ...
    private void OnGUI()
    {
        if (_manager == null || _serializedObject == null)
        {
    ...
    private void OnDrag(Vector2 delta)
    {
        ...
        ((FSM_Manager)_serializedObject.targetObject).DragAnyStateBox(delta);
    ...
    private bool TryToGetSerializedObject()
    {
        if (_manager == null)
        {
            _manager = EditorUtility.InstanceIDToObject(_instanceID) as FSM_Manager;
            if (_manager == null)
                return false;
        }

        _serializedObject = new SerializedObject(_manager);
    ...
    public void OnClickDeleteState(FSM_State state)
    {
        ...
        ((FSM_Manager)_serializedObject.targetObject).OnStateDeleted(deletedStateIndex);
        _serializedObject.ApplyModifiedProperties();
        _serializedObject.Update();
    ...
    private bool ProcessAnyStateEvents(Event e, bool alreadyCastBox)
    {
        ...
        switch (e.type)
        {
            ...
            case EventType.MouseDrag:
                ...
                {
                    ((FSM_Manager)_serializedObject.targetObject).DragAnyStateBox(e.delta);
                    e.Use();
                    return true;
                }
                break;
    ...
    {
        ((FSM_Manager)_serializedObject.targetObject).AddAnyStateTransition(endStateIndex);
        _serializedObject.ApplyModifiedProperties();
        _serializedObject.Update();
    }
    #endregion
